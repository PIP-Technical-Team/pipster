...
test_that("pipgd_validate_lorenz inputs and outputs", {
  ...
  params                  <- pipgd_params(welfare = pip_gd$L,
                                          weight  = pip_gd$P)
  res_with_params         <- pipgd_params(welfare = pip_gd$L,
                                          weight  = pip_gd$P) |>
    pipgd_validate_lorenz()

  res_with_welfare_weight <- pipgd_validate_lorenz(welfare = pip_gd$L,
                                                   weight  = pip_gd$P)
  ...
  pipgd_validate_lorenz(welfare = NULL,
                        weight  = NULL,
                        params  = test_params) |>
    expect_error()

  expect_no_error(
    q <- pipgd_validate_lorenz(welfare = pip_gd$L,
                               weight  = pip_gd$P)
  )
  ...
  expect_equal(
    0.3,
    q$gd_params$lq$validity$headcount
  )
  expect_error(
...
test_that("pipgd_lorenz_curve", {
  ...
  expect_equal(
    class(pipgd_lorenz_curve(welfare = welfare,
                             weight  = weight)),
    "list")

  # Lorenz input ---------------------------------------------------------------
  lorenz <- "lq"
  res_with_lorenz <- pipgd_lorenz_curve(welfare = welfare,
                                        weight  = weight,
                                        lorenz  = lorenz)
  expect_equal(
    res_with_lorenz$lorenz_curve$lorenz,
    lorenz)

  res_null_lorenz <- pipgd_lorenz_curve(welfare = welfare,
                                        weight  = weight,
                                        lorenz  = NULL)
  expect_equal(
    res_null_lorenz$lorenz_curve$lorenz,
    params$selected_lorenz$for_dist)

  invalid_lorenz <- "neither lq nor lb nor null"
  pipgd_lorenz_curve(welfare = welfare,
                     weight  = weight,
                     lorenz  = invalid_lorenz) |>
    expect_error()
  ...
  # Check params vs welfare ----------------------------------------------------
  expect_equal(
    pipgd_lorenz_curve(
      params  = pipgd_params(welfare = welfare,
                             weight  = weight))$lorenz,
    pipgd_lorenz_curve(
      welfare = welfare,
      weight  = weight)$lorenz)
